...
def liquidacion_mes(rut: str, fecha_desde: str, fecha_hasta: str, censura: int, folio: int):
    ...
    try:
        ...
        return f"Warning - Fecha Desde no cumple con el formato {error}"

    try:
        dateHasta = datetime.date(datetime.strptime(fechaHasta, '%Y-%m'))
    except Exception as error:
        return f"Warning - Fecha Hasta no cumple con el formato {error}"

    pdfliq = FPDF(orientation='P', unit='mm', format=(216, 330))
    pdfliq.add_font('Arial', '', 'c:/windows/fonts/Arial.ttf', uni=True)  # added line
    while dateDesde <= dateHasta:
        # contratos = []
        fecha_mes = str(dateDesde).replace('-', '').strip()[:-2]
        dateDesde = dateDesde + relativedelta(months=1)
        # print('Fecha mes', fecha_mes)
        try:
            contratos = listar_contratos(rut, fecha_mes)
            # print(contratos)
            if not contratos:
                return f"Warning - No existen contratos en la fecha indicada {fecha_mes} o el rut es incorrecto"
        except Exception as error:
            return error
        ...
        for contrato in contratos:
            ...
            try:
                ...
                if len(billetaje) > 0:
                    ...
                    return {'msg': 'No existe liquidacion para la fecha consultada.'}
            except Exception as error:
                return {'msg': error}
            finally:
                if cursor:
            ...
            try:
                ...
                if infocontrato:
                    ...
                    if str(rut_c).strip() == '':
                        return {'msg': 'No existe informacion del funcionario consultado. (revise tipo funcionario)'}
                else:
                    return {'msg': 'No es posible obtener informacion de contrato.'}
            except Exception as error:
                return {'msg': error}
            finally:
                if cursor:
            ...
            try:
                ...
                if numeropaginas > 0:
                    ...
                    return {'msg': 'no existe liquidacion'}
    ...
    # if contratos:
    if folio:
        ruta_archivo = 'pdf_files/' + folio + '.pdf'
        # print(ruta_archivo)
        pdfliq.output(name=ruta_archivo, dest='F').encode('latin-1')
    else:
        return f"Warning - No existe numero de folio"

    return f"Archivo Generado"


def delete_files_in_directory(directory_path):
    # Elimino todos los archivos que existan en el directorio
    try:
        ...
        for file in files:
            ...
            if os.path.isfile(file_path):
                os.remove(file_path)
        print("Todos los archivos fueron eliminados satisfactoriamente.")
    except OSError:
        print("Ha ocurrido un error al eliminar los archivos.")
...
def listar_contratos(rut, fecha):
    try:
        conn = conectorbd()
        cursor = conn.cursor()
        # print("EXEC BD_REMUNE.dbo.sp_listar_contratos " + rut + ", " + fecha)
